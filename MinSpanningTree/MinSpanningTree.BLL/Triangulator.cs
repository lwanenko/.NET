...
namespace MinSpanningTree.BLL
{
    class Triangulator
    {
        ...
        private void Analyse(List<Point> suppliedPoints, Hull hull, List<Triad> triads, bool rejectDuplicatePoints, bool hullOnly)
        {
            ...
            {
                distance2ToCentre[k] = points[0].DistanceTo(points[k]);
                sortedIndices[k] = k;
            }
            ...
            for (int k = 3; k < nump; k++)
                distance2ToCentre[k] = points[sortedIndices[k]].DistanceTo(centre);
        ...
        /// <returns></returns>
        public List<Point> ConvexHull(List<Point> points, bool rejectDuplicatePoints)
        {
            Hull hull = new Hull();
        ...
        public List<Triad> Triangulation(List<Point> points)
        {
        ...
        /// <param name="rejectDuplicatePoints">Whether to omit duplicated points</param>
        /// <returns></returns>
        public List<Triad> Triangulation(List<Point> points, bool rejectDuplicatePoints)
        {
            List<Triad> triads = new List<Triad>();
            Hull hull = new Hull();
